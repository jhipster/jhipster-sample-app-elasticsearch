...
export class BankAccountService extends BankAccountsService {
  ...
  search(req: Search): Observable<IBankAccount[]> {
    const options = createRequestOption(req);
    return this.http.get<IBankAccount[]>(this.resourceSearchUrl, { params: options }).pipe(catchError(() => scheduled([], asapScheduler)));
  }
